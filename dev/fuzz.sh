...
if [ "$(uname)" = Darwin ]; then
    ...
    nix-shell -E '{ pkgs ? import <nixpkgs> {} }: (pkgs.mkShell.override { stdenv = pkgs.llvmPackages.stdenv; }) { packages = with pkgs; [ brotli fontconfig freetype glslang harfbuzz llvmPackages.lld pkg-config python3 ragel simdutf spirv-cross utf8proc ]; }' \
