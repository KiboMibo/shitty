...
MouseProtocolPoint mouseProtocolPoint(MouseTrackingEnc encoding, int pixelX, int pixelY, const MouseGeometry& geometry) {
    ...
    const int contentHeight = max(1, geometry.framebufferHeight - 2 * geometry.border - geometry.topInset);
    if (encoding == MouseTrackingEnc::SGRPixels) {
        return {
            min(max(pixelX - geometry.border + 1, 1), contentWidth),
            min(max(pixelY - geometry.border - geometry.topInset + 1, 1), contentHeight),
        };
    }
    ...
    return {
        ...
        min(max((pixelY - geometry.border - geometry.topInset) / max(1, geometry.glyphHeight) + 1, 1), rows),
    };
}
