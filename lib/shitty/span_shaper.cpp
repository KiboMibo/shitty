...
bool SpanShaperImpl::sixelCell(const TerminalCell& cell) const {
    return cell.hasExtra() && composer.cellExtras != nullptr && composer.cellExtras->view(cell).sixelPixels != nullptr;
}
...
size_t SpanShaperImpl::arenaBudget(size_t pixel) const {
    const size_t viewport = (size_t)(composer.columns) * composer.rows;
...
u32 SpanShaperImpl::renderSixelStrip(const TerminalCell* cells, u16 begin, u16 end) {
    ...
    const size_t stride = (size_t)(spanCells)*cellWidth;
    for (u16 column = begin; column < end; ++column) {
        const CellExtraView view = composer.cellExtras->view(cells[column]);
        ...
        if (sixelCell(cells[column])) {
            ...
            if (out != nullptr) {
                ...
                for (u16 cell = begin; cell < end; ++cell) {
                    const CellExtraView view = composer.cellExtras->view(cells[cell]);
...
SpanShaper* SpanShaper::create(Composer& composer, ObjPool& pool) {
    ...
    composer.cellExtrasChangedListeners.pushBack(pool.make<CallShaperExtrasCollected>(shaper));
    return shaper;
}
