...
namespace {
    class ApplicationImpl final
        ...
        , public PtyEventHost {
    public:
    explicit ApplicationImpl(Composer& composer_)
        : composer(composer_)
    {
    }

    ~ApplicationImpl() { emergencyCleanup(); }
    ...
    bool present(const Frame& frame) override {
        refreshPending = true;
        if (!refreshAllowed || !renderer->update(frame)) return false;
    ...
    bool handlesOsc() const override { return terminalHostReady; }

    void bell() override { glfwRequestWindowAttention(window); }
    ...
    void wake() override { glfwPostEmptyEvent(); }
    ...
    void setupCallbacks() {
        ...
        glfwSetKeyCallback(window,
            [](GLFWwindow* w, int key, int scancode,
               int action, int modifiers) {
        ...
        glfwSetMouseButtonCallback(window,
            [](GLFWwindow* w, int button, int action, int modifiers) {
                fromWindow(w).onMouseButtonCallback(
                    w, button, action, modifiers);
            });
    ...
    int run(int argc, char* argv[]) override {
        ...
        PtyEventSource* ptySource = PtyEventSource::create(
            composer, *terminalPty, *this);
