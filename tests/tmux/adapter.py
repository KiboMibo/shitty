...
def main():
    ...
    with (
        Zutty(columns=80, rows=25, save_lines=100) as whole,
        Zutty(columns=80, rows=25, save_lines=100) as chunked,
    ):
        for member in members:
            ...
            if member in known_failures:
                ...
                print(
                    "single member: python3 tests/tmux/adapter.py "
                    "tests/tmux/xfail.txt /tmp/tmux.stamp " + member,
                    file=sys.stderr,
                )
                return 1
