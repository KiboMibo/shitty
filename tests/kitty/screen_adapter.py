...
def rendered_line(snapshot, row):
...
def main():
    ...
    timeout_message = f"FAIL Kitty-screen/{name}: exceeded 10 seconds\n".encode()
    ...
    signal.alarm(10)
    label, row, columns, rows, payload, expected = case_data(name)
    with Zutty(columns=columns, rows=rows, save_lines=0) as terminal:
        terminal.write(payload)
        actual = rendered_line(terminal.snapshot(), row)
    normalized_expected = expected.rstrip(" ")
    mismatch = None if actual == normalized_expected else (
        f"line {row} got {actual!r}, expected {normalized_expected!r}")
    signal.alarm(0)
    if (mismatch is not None) != (name in known):
        status = "FAIL" if mismatch is not None else "XPASS"
        print(f"{status} Kitty-screen/{name} ({label}): "
              f"{mismatch or 'matched'}", file=sys.stderr)
        return 1
    print(f"{'XFAIL' if mismatch else 'PASS'} Kitty-screen/{name} ({label})"
