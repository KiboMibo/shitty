...
def main():
    ...
    if (mismatch is not None) != (name in known):
        ...
        print(f"{status} WindowsTerminal/{name}: {mismatch or 'matched'}", file=sys.stderr)
        return 1
    if mismatch is not None:
        print(f"XFAIL WindowsTerminal/{name}: {mismatch}")
    else:
        print(f"PASS WindowsTerminal/{name} ({len(payload)} bytes)")
