...
Zutty ignores Delete Character when the cursor is outside the vertical margins. This agrees with an older Digital Equipment Corporation description, but the practical `xterm-256color` profile behaves differently:
...
Conclusion: **fix Zutty and remove the expected failure**. The current comment in `tests/esctest/xfail.txt` defends the Digital Equipment Corporation behavior, but that behavior conflicts with both the xterm profile selected by Zutty and the behavior of the principal terminal emulators.
...
Zutty follows the literal Digital Equipment Corporation rule that the operation is ignored at the page boundary. Modern implementations shift the contents when the cursor is at an active margin, including when that margin coincides with the screen boundary:
...
None of these sixteen cases identifies a Zutty bug.
...
- `selective_erasure`: xterm, Ghostty, Contour, xterm.js, and Windows Terminal support Select Character Protection Attribute, Selective Erase in Display, and Selective Erase in Line. Alacritty, Kitty, VTE, and WezTerm either do not support them or support them only partially. Zutty implements the actual feature.
...
For an xterm profile, Zutty behavior is correct. These expected failures may remain, although a future adapter should preferably compare rendered behavior instead of invisible internal cell fields.
...
- `vttest.11.6.6.vt102-with-bce`: two checkpoints were recorded in a different execution phase. Two more keep the cursor column after Insert Line or Delete Line even though vttest itself states that the cursor should move to the first column. Zutty and xterm follow that stated behavior.
- `vttest.11.6.7.ecma48-misc-with-bce` and `vttest.11.7.iso6429-other`: these execute Repeat after a previous Repeat control function. The fifth edition of the European Computer Manufacturers Association standard ECMA-48 leaves Repeat behavior undefined when the preceding item is a control function. Contour stops after the first repetition; Zutty, Kitty, Ghostty, and Konsole continue repeating the last graphic character. Windows Terminal and xterm.js clear Repeat state after a control function. Both policies are permitted.
- `vttest.11.6.7.1.protected-area-with-bce` and `vttest.11.7.1.protected-area`: the test enables Erasure Mode number 6. In the set state, ECMA-48 requires ordinary erase operations to erase guarded cells as well. Zutty does so, while the Contour golden result retains the box. The golden result is incorrect.

Conclusion: **do not change Zutty for these cases**. The comments should instead be split by individual checkpoint.
...
xterm.js and VTE provide only the restricted private mode 45 behavior. WezTerm and foot still assign the older, less restricted behavior to private mode 45. The old esctest cases expect that behavior without enabling private mode 1045. **Zutty agrees with modern xterm, so these expected failures should remain.**
...
- xterm, VTE, and Zutty use the space character, Unicode code point U+0020, or an undefined blank.
...
- Parser trace: libvterm canonicalizes a seven-bit escape representation of a C1 control into a C1 callback. Zutty preserves the actual parser grammar production. User-visible terminal behavior is identical.
...
- `escape_nf` and `escape_fpes`: VTE interprets Escape followed by `Z` as the European Computer Manufacturers Association Single Character Introducer and waits for a following character. xterm, Ghostty, Kitty, Windows Terminal, and Zutty recognize the complete VT100 Identify Terminal command. VTE is the outlier for this terminal profile.
- Twelve mixed Operating System Command cases: VTE requires the introducer and terminator to use matching seven-bit or eight-bit forms. xterm, Zutty, and most modern parsers accept a seven-bit Operating System Command followed by an eight-bit String Terminator, and the reverse combination. The European Computer Manufacturers Association standard does not prescribe malformed-stream recovery here.
- Oversized Operating System Command: maximum lengths are implementation policy:
  ...
  - Zutty allows one mebibyte.
  - xterm.js allows ten million bytes.
  - foot grows the buffer dynamically.

The Zutty limit is reasonable. **Keep all 14 expected failures.**
...
Zutty follows default xterm behavior. **Keep the expected failure.**

## xterm.js: 8 expected failures

- Vertical Position Backward: xterm.js ignores the standardized `Control Sequence Introducer Ps k` operation. Zutty, VTE, and Contour implement the movement. Zutty is correct.
- Repeat after Line Feed or Cursor Backward: the standard leaves the result undefined, and terminal implementations are divided. Keep the selected Zutty behavior.
- Save Cursor and Save Current Cursor Position with pending wrap: xterm, Ghostty, Alacritty, and Zutty preserve the wrap flag. The xterm.js golden result loses it.
- Set Top and Bottom Margins quirks: Ghostty and Zutty reject excess parameters. Kitty and xterm.js generally use the first two parameters. The Digital Equipment Corporation definition specifies exactly two parameters, so Zutty is correct.
- Cursor Backward Tabulation with pending wrap: the standard requires movement to the previous tab stop. xterm.js ignores the operation because its internal cursor is represented as being one column beyond the screen. Zutty is correct.
