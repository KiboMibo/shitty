...
namespace {
    ...
    struct Rig {
            ...
            : pool(ObjPool::fromMemory())
        {
            ...
            term = VtermHeadless::create(*composer, &capture, pty)->terminal();
            api = capture.api;
        }
    };
    ...
    static bool equalHyperlink(const Rig& a, const Rig& b, u16 row, u16 column) {
        const int x = a.composer->vt.borderPixels() + column * a.composer->vt.glyphWidth;
        const int y = a.composer->vt.borderPixels() + row * a.composer->vt.glyphHeight;
        ...
        switch (op) {
            ...
            case 217:
                if (len >= 2) {
                    ...
                    rig.composer->vt.resize((u16)(2 * rig.composer->vt.borderPixels() + columns * rig.composer->vt.glyphWidth), (u16)(2 * rig.composer->vt.borderPixels() + rows * rig.composer->vt.glyphHeight));
                }
                break;
            case 218:
                if (len >= 1) {
            ...
            case 221:
                if (len >= 2) {
                    ...
                    rig.composer->vt.resize((u16)(2 * rig.composer->vt.borderPixels() + columns * rig.composer->vt.glyphWidth), (u16)(2 * rig.composer->vt.borderPixels() + rows * rig.composer->vt.glyphHeight));
                    rig.composer->vt.resize(backWidth, backHeight);
                }
                break;
            case 222:
                if (len >= 2) {
                    ...
                    rig.composer->vt.resize((u16)(2 * rig.composer->vt.borderPixels() + rig.composer->vt.columns * glyphWidth), (u16)(2 * rig.composer->vt.borderPixels() + rig.composer->vt.rows * glyphHeight));
                }
                break;
            case 225: {
