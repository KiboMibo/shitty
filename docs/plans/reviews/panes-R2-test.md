...
# Перенос после F2e (2026-08-19)

`F2e` (`d7a0cf6d`…`39b054b0`, дерево на `749cb31f`) закрыла находки этого отчёта; тесты
перенесены в рабочее дерево коммитом `88ada3be` — **12 из 13**, один выброшен.

## Статус находок после F2e

| Находка | Статус | Чем закрыта |
|---|---|---|
| `L1` сборка на не-Apple | **закрыта** | Cocoa-ветка `applySavedQuickFrame()` под `#if defined(__APPLE__)` (`a62cb7ab`) |
| `I11` pty-хелпер не компилируется | **закрыта** | `c9b5b361`; `./build test` теперь исполняет 18 узлов `[UT]` вместо нуля |
| `I13` пересчёт сетки не покрыт | **закрыта** | `quickFrameRegridExtent()` вынесена в `quick_frame_store.{h,cpp}` и покрыта пятью тестами (`d3672f2b`). Перепроверено: **мутация `M14`, не убивавшая ничего, теперь убивает** — арифметика вышла из недостижимой ветки |
| `Z4` висячий `quickToggleEvent` | **закрыта** | `a62cb7ab` |
| `Z5` ключ не стрипуется | **закрыта** | `parseFrameLine()` стрипует ключ, тест `SpacesAroundTheSeparatorDoNotSpoilTheFile` (`d3672f2b`) |
| `Z6` формулировка `F2c` про `TargetLeavesAFittingFrameAlone` | принята командиром, правится в борде и отчёте `F2c` |
| `I12` два падения `Pty` | **оставлена намеренно** — разбор вне волны 2 |
| `I14` тестовый бинарь с живым `NSWindow`, `Z7` инструментация покрытия | **отложены** с обоснованием в борде |

## Поправка к методике `L1` — важнее самой находки

Мой эксперимент («убрать `ui_quick_hotkey.mm` из графа и посмотреть на линкер») **не
различал состояние до и после**: на macOS `__APPLE__` остаётся определён, поэтому и
защищённый гвардом `createQuickHotkey()`, и незащищённый `applyQuickFrameToWindow()` дают
одинаково неразрешённый символ — вывод линкера побайтово тот же и на исправленном дереве.
Находка была верна, но подтверждена она была не этим.

Различающая проверка — та, которую сделала `F2e`: аудит вложенности директив плюс поиск
вызовов darwin-only символов **вне** блока `__APPLE__`; до правки таких вызовов 1, после 0.
Кто будет проверять похожее на следующих волнах — воспроизводить надо это, а не мой
эксперимент.

## Что перенесено (12)

Все двенадцать перепроверены мутациями **на коде `F2e`**, а не на старом: мутация вносилась
в отдельный worktree `/tmp/r2test2` на `749cb31f`, собиралась и откатывалась.

| Тест | Файл | Мутация, которая его роняет |
|---|---|---|
| `TargetSurvivesADegenerateScreenRect` | `quick_frame_store_ut.cpp` | `M3`: убрать `max(1.0, …)` для размеров экрана |
| `TargetNeverLetsTheTitlebarShrinkTheFrame` | там же | `M17`: `max(0.0, titlebarHeight)` → `titlebarHeight` |
| `TargetClampsThePositionAgainstTheFrameHeightIncludingTheTitlebar` | там же | `M1`: убрать `+ titlebar` (роняет заодно три прежних теста) |
| `NegativeSizeIsTreatedAsAbsent` | там же | `M4`: снять `value >= 0` |
| `FileMixingOldAndNewKeysIsTreatedAsAbsent` | там же | `M5`: частичный алиас старых ключей |
| `RepeatedKeyTakesTheLastValue` | там же | `M19` (новая): первое значение вместо последнего |
| `BlankAndUnrecognizedLinesDoNotSpoilAnOtherwiseCompleteFile` | там же | `M20` (новая): убрать `stripCr()` |
| `SaveLeavesAnotherProcessTemporaryFileAlone` | там же | `M6b`: временный файл без `pid` в имени |
| `WriteFailureAfterOpenRemovesTheTemporaryFile` | там же | `M7`: не удалять временный файл при ошибке записи |
| `ApplyingASavedFrameRefusesANonCocoaBackend` | `application_ut.cpp` | `M16`: снять проверку тега бэкенда — бинарь падает по `SIGSEGV` на этом тесте |
| `ApplyingASavedFrameRefusesAComposerWithNoWindow` | там же | та же `M16` (проходит до краша соседа) |
| `CompanionKeepsAFrameStoreSeparateFromItsParent` | `quick_companion_ut.cpp` | `M18`: не выставлять `selfReference`; `M21` (новая): выводить имя файла состояния из директории, а не из имени конфига |

**Переписан один.** `BlankAndUnrecognizedLinesDoNotSpoilAnOtherwiseCompleteFile` в
исходной версии проверял ещё и пробелы вокруг `=` — вернее, фиксировал, что строка с
пробелом перед `=` **теряется**. `F2e` это поведение исправила (`Z5`) и покрыла своим
`SpacesAroundTheSeparatorDoNotSpoilTheFile`; моя версия свелась к тому, чего у неё нет —
CRLF, пустые строки, нераспознанные строки, — а комментарий про «пробел перед `=` не
проверяем» убран, он описывал уже несуществующее ограничение.

Остальные одиннадцать перенесены дословно: `quickFrameTarget()`, парсер и `saveQuickFrame()`
`F2e` по существу не трогала, а ранние возвраты `applyQuickFrameToWindow()` на месте.

## Что выброшено (1) и почему

`ShowKeepsTheDefaultPlacementWhenThereIsNoConfigPath` — **нечувствителен, измерено**.
Мутация «убрать ранний возврат при пустом `configPath`» (`defaultQuickFramePath()`
вызывается, результат игнорируется) не роняет его: `loadQuickFrame("")` всё равно вернёт
`false`, и поведение совпадает до последнего пикселя. Прогон с этим тестом при мутации —
`OK: 14`, ни одного падения. Он покрывал строку, а не поведение, и `ShowWithNoSavedFile­KeepsTheDefaultPlacement`
проверяет ту же наблюдаемую развилку через несуществующий путь конфига. Ещё в первом
отчёте он был помечен как «покрытие ветки, а не чувствительный тест» — теперь это
подтверждено числом, и держать его незачем.

Тестов `F2e` я не дублировал: её 19 покрывают `quickFrameOverlap()`,
`quickFrameFitsScreens()`, `quickFrameShouldSave()` и `quickFrameRegridExtent()` — всё, что
она вынесла в чистые функции; ни один мой тест туда не лез.

## Базис после переноса

| Что | Значение |
|---|---|
| `./build -j 8 st pt st_test unit_tests plt_unit_tests` | `exit 0` |
| `.build/unit_tests` | базис **716 OK / 3 ERR** → **728 OK / 3 ERR** (ровно +12), одинаково в оба режима планировщика |
| То же с `SHITTY_PTY_TEST_HELPER` | базис 717/2 → **729 OK / 2 ERR** |
| `.build/ext/plt/plt_unit_tests` | **54 OK / 0 ERR** |
| `clang-format -lines` по каждому из шести новых диапазонов | чисто (`dev/style.py` не запускался — он форматирует файлы целиком) |

## Мой companion-тест был зелёным по двум ложным причинам — переписан

Перенесённый дословно, он **упал в графе** (`$(B)/unit-tests/group-15.stamp`,
`StringView(ownFrame) != StringView(companionFrame) failed`), хотя вне графа проходил.
Обе причины стоит знать, потому что обе — не про продукт:

1. `makeTempFile()` даёт имена вида `quick_companion_ut.AbC123`, различающиеся **только
   после последней точки** — ровно той части, которую `defaultQuickFramePath()` заменяет.
   Под `TMPDIR` графа оба конфига честно сходились в один файл состояния, и тест поймал
   собственную постановку, а не дефект.
2. Он сравнивал сырой путь с `realpath()`-нутым. На macOS `TMPDIR` ведёт к одному каталогу
   и через `/var`, и через `/private/var`, поэтому пути расходились по причине, не имеющей
   отношения к именам конфигов. Проверено мутацией `M21`: тест оставался **зелёным**, даже
   когда файл состояния переставал зависеть от имени конфига вообще.

Переписан: два конфига — документированная пара `shitty.toml` + `quick.toml`
(`bin/st/shitty.toml:104`) в одном временном каталоге, оба пути канонизируются перед
сравнением. После этого `M21` его роняет, `M18` тоже, а узел `group-15` в графе зелёный.
Коммит `14d86912`.

## Сверка `./build test -k` с новым базисом

**13 упавших именованных узлов** — ровно базис после `F2e`:
...
`$(B)/unit-tests/group-{13,14}.stamp` (оба — `Pty`, находка `I12`). Узлов `[UT]` — 17-18,
против нуля до `F2e`. Регрессий ноль.
