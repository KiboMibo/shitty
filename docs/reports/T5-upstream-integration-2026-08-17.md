...
Тест `ShiftedPrintableKeepsBothLayoutLevelsOnRelease`, который team lead
называл эталонным индикатором раскладки (12 vs 11 узлов), в дереве не найден
ни по какому пути (`grep -r` по всему репозиторию — пусто) ни на одной из
веток; возможно, это динамически генерируемое имя из другого прогона среды.
Не смог подтвердить/опровергнуть этот конкретный пункт — требует уточнения у
team lead, чем он был получен.
...
- Тест `ShiftedPrintableKeepsBothLayoutLevelsOnRelease` не найден в дереве —
  не смог подтвердить его статус ни на одной из веток, нужно уточнение у
  team lead об источнике.
