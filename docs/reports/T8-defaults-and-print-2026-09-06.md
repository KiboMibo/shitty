...
**Дописано `T8b`, 2026-09-06, после мержа обоих.** Дефект починен `F-panes`
(`e075a14f`): `pointerPresence(false)` больше не зовёт `dropPointerGrab()`,
потому что отпускание кнопки, зажатой при выходе за край, — ровно тот жест,
конец которого окно **гарантированно** увидит. Пин `+panes` снят: с `panes =
true` по умолчанию это путь, по которому идёт почти всякое пользовательское
перетаскивание, и пин прятал бы от теста именно тот код, который его несёт.

Тест проверен как настоящий наблюдатель, а не как «зелёный на зелёном»
**[и]**. Мутация — `dropPointerGrab()` возвращён в `pointerPresence(false)`,
`st_test` пересобран:
...
Той же мутацией краснеет ровно один узел `unit_tests` —
`SessionSet::AReleaseAfterThePointerLeftTheWindowStillEndsInThePaneThatTookThePress`
(`session_ut.cpp:2594`), заведённый `F-panes`; `OK: 987, ERR: 1`. То есть у
места два независимых наблюдателя — C++ на уровне сессий и питоновский на
уровне протокола, — и оба зелены до мутации, что и есть условие из `CLAUDE.md`
(«назвать наблюдателя поимённо и убедиться, что он зелёный до мутации»).

Мутация откачена, `git status` по `session.cpp` чист; после отката
`unit_tests` — `OK: 988`, `EXIT=0`, питоновский набор — `Ran 6613`,
`errors=14`, `skipped=23`, поимённо те же четырнадцать.
