...
### A3. Арены глифов — на окно, и это измеряется, а не предполагается

- **Развилка:** у каждой сессии свой `Screen` со своим поколением спанов, а в
  рендерере поколение одно (`stripGeneration`).
- **Решение (пересмотрено 2026-09-03, см. ниже):** арены глифов ведутся на
  окно: **один `SpanShaper` на окно — одна арена на плоскость**, и смещения
  стрипов, которые раздаёт панель, уже являются смещениями в неё. Панель не
  владеет ни диапазоном, ни базой.
  В исходной редакции решение читалось иначе: «арены глифов ведутся
  **диапазонами на панель**; поколение сравнивается в паре с идентификатором
  панели. Пока диапазоны не введены, сплиты не включаются». Ни диапазонов, ни
  `PaneArenaMirror` в дереве больше нет.
...
- **Механизм пересмотрен 2026-09-03, апстримным мержем** (коммит `2a11ffa7`
  «Metal mirrors the window's one arena, and the pane ranges go»; в документ
  внесено `F-A7` 2026-09-05, основание — ревью `T7.3`,
  `docs/plans/reviews/upstream-merge-R8-arch.md`, находка 2; исход «документ
  устарел» был **предусмотрен планом** `docs/plans/2026-08-29-upstream-merge.md`
  и назван условием закрытия в чек-листе `T0.3`).
  - **Что стало.** `PaneArenaMirror` снят; `lib/shitty/render_arena.h`
    переписан в `ArenaMirror` — зеркало **окна**, а не панели, с контрактом в
    две строки: пока поколение держится, слать хвост; когда сдвинулось, слать
    всё. `render_arena_ut.cpp` сжался с двенадцати тестов до пяти.
  - **Прежний запрет снят не тем доводом, которым он был поставлен.** Запрет
    2026-08-19 касался довода «поколения уникальны» — и он остаётся в силе: на
    уникальность номеров опираться по-прежнему нельзя. Мерж опёрся на другое:
    **арена одна**, поэтому путать нечего. Это верно ровно до тех пор, пока
    шейпер один.
  - **Допущение не исчезло, оно сузилось и стало неохраняемым типами:** «**один
    ...
    своим — делает зеркало неверным тем же тихим способом: две арены, две
    последовательности поколений, одно зеркало, отвечающее «ничего не
    сдвинулось» про байты, которые лежат в другом месте.
  - **Наблюдателя у этого допущения нет ни одного** — ни теста, ни гварда
    (измерено `T7.3`). Единственное, что стоит на его месте, — абзац в
    `render_arena.h`, называющий владельца: «кто заведёт второй шейпер, тот
    владеет этим файлом». Кто заводит второй шейпер, обязан завести и
    наблюдателя.
  - **Смежное допущение — пер-процессная идентичность строк** — наблюдателя
    имеет, и ровно одного: `Screen::RowIdentitiesNeverRepeatAcrossScreens`
    (`screen_ut.cpp`). `T7.3` измерила его границу: счётчик, сделанный членом
    экрана, тест краснит, а счётчик, **сбрасываемый в ноль** в
    `Screen::createPrimary()`, — нет, потому что тест создаёт оба экрана и
    только потом пишет в них вперемежку. Тест был бы устойчив и ко второй форме,
    если бы писал в первый экран **до** создания второго.
...
- **Решение (пересмотрено 2026-08-25, см. ниже):** полоса титлбара **не
  резервируется вовсе** — `contentInsets` не несёт `Top` ни при включённом
  режиме, ни при выключенном, — а наведение переключает **только видимость**
  оформления, нарисованного **поверх** верхних строк. Число строк сетки при
  этом неизменно, и это и есть гарантия `A7`.
  В исходной редакции решение читалось иначе: «полоса титлбара **резервируется
  постоянно** (входит в `contentInsets` всё время, пока режим включён)». Та
  редакция описывает механизм, которого в продукте больше нет; гарантия — та же.
...
- **Механизм пересмотрен 2026-08-25** (правка `C11`, коммит `6628ba59`
  «autoHideChrome: вкладки и текст у верхней границы окна»; в документ внесено
  `F-A7` 2026-09-05, основание — ревью `T7.3`,
  `docs/plans/reviews/upstream-merge-R8-arch.md`). **Это не регрессия, а
  сознательная правка по жалобе пользователя** — следующий читатель, не найдя
  резерва, не должен искать, кто его потерял.
  - **Что было написано:** резерв под титлбар входит в `contentInsets` всё
    время, пока режим включён, и ховер переключает только видимость.
  - **Как на самом деле:** резерв был единственным, что двигало и сетку
    терминала, и список вкладок сайдбара (его отступ читает тот же резерв), и
    пустая полоса над первой строкой — то, что человек назвал некрасивым.
    Теперь `applyAutoHideChrome()` передаёт **литеральный ноль**
    (`lib/shitty/ui_csd_tabs.mm:309`), титлбар рисуется поверх верхних строк на
    тех кадрах, где он вообще виден, а `chromeStripPoints()`, мерявший его
    высоту, удалён вместе со своим тестом.
  - **Почему решение всё равно верное.** Раздел «Почему» выше не изменился ни в
    одном слове и остаётся главным риском фичи: `SIGWINCH` на каждом пересечении
    границы неприемлем. Изменился **способ**, которым риск снят: не «резерв
    постоянный ненулевой», а «резерв постоянный нулевой». Постоянство — то, что
    несёт гарантию; величина — нет.
  - **Цена, которую надо знать.** Гарантия теперь держится **тривиально**:
    двигать нечего, поэтому и не двигается. Измерено `T7.3`: мутация «ховер
    удваивает найденный резерв на входе и делит пополам на выходе» — ровно то
    нарушение, ради которого `A7` заведена, — проходила **весь набор зелёным**.
    `F-A7` перенацелила единственного прямого наблюдателя
    (`CsdTabsUi::HoverChangesVisibilityAndNothingElse`): тест сам ставит
    ненулевой резерв, утверждает, что его двойник, его половина и ноль
    **различимы** по числу строк, и только потом водит указатель. Обе мутации
    краснят его поимённо.
  - **Оставшийся риск и запрет.** Единственное, что здесь ещё может сломаться, —
    возврат **ненулевого** резерва `Top` в режиме автоскрытия: вместе с ним
    вернётся `SIGWINCH` на каждом движении мыши. Не возвращать, не «восстановив»
    заодно ту редакцию решения, которую этот пункт отменяет.
  - **Следствие для `A1`.** В продукте `setChromeReserve()` зовётся **дважды**:
    сайдбар (`lib/shitty/ui_sidebar_tabs.mm:676`, сторона `Left`, настоящая
    величина) и этот ноль. Асимметрия резерва по сторонам, которую стережёт
    `A1`, нагружена в продукте **одной** стороной из четырёх; остальные три
    проверяются только тестами. Это не дефект, но запас прочности `A1` меньше,
    чем читается из документа.
